...
While our form of REST met our needs, when I looked around at the REST community at large, I felt that we were mostly alone. Many talked about hypermedia as the right way to build APIs, and some specs and API platforms delivered linking requirements and technology to bring them to life. Nonetheless, it seemed to me that people were still developing “level 2” APIs with few links between resources.

Many have concluded that REST needs an opinionated set of resource profiles and patterns to give API clients and developers a language above the HTTP protocol. We need a way out of the genuinely tired arguments over the meaning of status codes and media types.
